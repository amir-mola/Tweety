...
def train(net, dataloader,val_loader, epochs=1, lr=0.01, momentum=0.9, decay=0.0005, verbose=1):
  ...
  for epoch in range(epochs):
    ...
    sum_loss = 0.0
    if(epoch % 2 == 1):
      torch.save(model.state_dict(), 'trained-vgg-{}.pth'.format(epoch))
    net.train()
...
train_loader, label_classes = get_data_loaders("train/", 128)


print("loading the weights")

model = models.vgg19_bn(pretrained=True)
model.classifier[6] = nn.Linear(model.classifier[6].in_features, 555)

print("Starting to train")
losses = train(model, train_loader, None, epochs=15)
print("Finished training")
torch.save(model.state_dict(), 'trained-vgg-15.pth')
